    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium">Publicá novedades, eventos y comunicados institucionales.</p>
        </div>
        <Link 
      ...
      <div className="bg-white rounded-[3rem] shadow-sm border border-slate-100 overflow-hidden">
        ...
        {noticias.length === 0 ? (
          <div className="py-20 text-center">
            ...
            <h4 className="text-lg font-bold text-slate-900">No hay noticias publicadas</h4>
            <p className="text-slate-500 mt-2">Empezá creando tu primera comunicación.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-slate-50">
                  <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Noticia</th>
                    ...
                    <td className="px-8 py-6">
                      <div className="flex items-center space-x-4">
                        <div className="h-14 w-20 rounded-xl bg-slate-100 overflow-hidden shrink-0 border border-slate-200">
                          {n.imagen_url ? (
                            <img src={n.imagen_url} alt="" className="h-full w-full object-cover" />
                          ) : (
                        ...
                        <div>
                          <p className="font-black text-slate-900 group-hover:text-blue-600 transition-colors line-clamp-1">{n.titulo}</p>
                          <p className="text-xs text-slate-400 font-medium line-clamp-1">{n.resumen || "Sin resumen"}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-6">
                      <span className={cn(
                        ...
                        n.publicada ? "bg-green-100 text-green-600" : "bg-orange-100 text-orange-600"
                      )}>
