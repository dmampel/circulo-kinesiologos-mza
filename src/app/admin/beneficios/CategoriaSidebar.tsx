...
export default function CategoriaSidebar({ categorias }: Props) {
  ...
  // Sincronizar campos cuando se entra en modo edición
  useEffect(() => {
    if (editingCat) {
                        ...
                        {COLOR_OPTIONS.map((color) => (
                          <button
                            ...
                            className={cn(
                              ...
                              `bg-${color}-500`,
                              selectedColor === color ? "border-white ring-2 ring-slate-900" : "border-transparent"
                            )}
                          />
                        ))}
                      </div>
                      ...
                      return (
                        <div key={cat.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl group">
                          <div className="flex items-center space-x-3">
                            <div className={cn("p-2 rounded-lg text-white", `bg-${cat.color || "slate"}-500`)}>
