...
export default async function CircularesAdminPage() {
  const circulares = await CircularRepository.getAll();
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium">Publicá avisos administrativos y comunicaciones exclusivas para socios.</p>
        </div>
        <Link 
      ...
      <div className="bg-white rounded-[3rem] shadow-sm border border-slate-100 overflow-hidden">
        ...
        {circulares.length === 0 ? (
          <div className="py-20 text-center">
            ...
            <h4 className="text-lg font-bold text-slate-900">No hay circulares publicadas</h4>
            <p className="text-slate-500 mt-2">Creá el primer comunicado para tus socios.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-slate-50">
                  <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Circular</th>
                  ...
                  <tr key={c.id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="px-8 py-6">
                      <p className="text-xs font-bold text-slate-400 flex items-center uppercase tracking-wider">
                        ...
                        {c.publicada_en ? new Date(c.publicada_en).toLocaleDateString("es-AR") : new Date(c.createdAt).toLocaleDateString("es-AR")}
                      </p>
                    </td>
