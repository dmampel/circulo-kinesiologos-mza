...
- [ ] 2.9 Desplegar y medir: repetir las mediciones de 1.2 y confirmar `x-vercel-cache: HIT` en la segunda request de cada ruta del grupo, y que `cache-control` ya no contiene `no-store` ni `private` — **PENDIENTE: requiere que la usuaria despliegue.** El comando exacto para correr después del deploy está en `baseline.md`.
