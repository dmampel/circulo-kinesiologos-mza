...
- [ ] 9.4 Prueba manual con Resend configurado: abrir una solicitud **vieja** (creada antes de `notificacion-solicitud-completa`), tocar "Reenviar aviso", verificar que llega a `INSTITUTIONAL_EMAIL` con los nueve campos, abrir cada enlace de documento en ventana privada sin sesión, y tocar el botón de WhatsApp. — **Pendiente**: requiere Resend real y un navegador; no ejecutable desde este entorno de apply. Queda para verificación manual antes de archivar.
- [ ] 9.5 Prueba manual: reenviar sobre una solicitud `APROBADA` y confirmar que funciona y que su status no cambió. — **Pendiente**, mismo motivo que 9.4.
- [ ] 9.6 Marcar todas las tareas con `[x]` antes de archivar el change (convención de `AGENTS.md`). — Todo marcado salvo 9.4/9.5 (pruebas manuales pendientes de ejecución humana).
